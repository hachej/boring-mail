...
const SELECTED_OLDER_THAN_500 = 1500
...
try {
  ...
  `)
  const hostileSubject = `Subject " \\ 😀 `.repeat(200_000)
  const hostileSender = `Sender " 😀 `.repeat(100_000)
  writer.prepare(`UPDATE participants SET display_name=? WHERE id=1`).run(hostileSender)
  writer.exec(`WITH RECURSIVE n(x) AS (
    ...
    SELECT x,1,1,'<bench-'||x||'@example.invalid>','email',datetime('2040-01-01','-'||x||' seconds')||'+00:00',
      'subject '||x,2,1,20,
      CASE WHEN x%5=0 THEN 'deleted' ELSE NULL END,
      CASE WHEN x%7=0 THEN 'deleted' ELSE NULL END
    FROM n`)
  writer.exec(`UPDATE messages SET deleted_at=NULL,deleted_from_source_at=NULL,sender_id=1 WHERE id=${SELECTED_OLDER_THAN_500}`)
  writer.exec(`UPDATE messages SET sent_at='2039-01-01 00:00:00+00:00' WHERE id BETWEEN 5000 AND 6500`)
  writer.exec(`UPDATE messages SET sent_at=NULL WHERE id BETWEEN 17000 AND 19000`)
  writer.prepare(`UPDATE messages SET subject=?,sender_id=1 WHERE id=1 OR id=?`).run(hostileSubject, SELECTED_OLDER_THAN_500)
  writer.exec(`WITH RECURSIVE n(x) AS (
  ...
  for (const id of [...Array.from({ length: 25 }, (_unused, index) => index + 1), SELECTED_OLDER_THAN_500]) {
    body.run(id, id === 1 || id === SELECTED_OLDER_THAN_500 ? hostileBody : `body ${id}`, '<b>html</b>')
  }
  ...
  const fanoutMessages = [...Array.from({ length: 25 }, (_unused, index) => index + 1), SELECTED_OLDER_THAN_500]
  for (const message of fanoutMessages) {
    for (let fanout = 1; fanout <= 24; fanout++) {
      const id = message * 1000 + fanout
      recipient.run(id, message, 2, fanout % 3 === 0 ? 'bcc' : fanout % 2 === 0 ? 'cc' : 'to', `Recipient " 😀 ${fanout}`.repeat(100), `r-${message}-${fanout}@example.invalid`)
      attachment.run(id, message, `file-${message}-${fanout}-"-😀`.repeat(100), `application/x-${'m'.repeat(300)}`, fanout, `hash-${id}`, `/private/${id}`)
    }
  }
} finally {
  writer.close()
...
try {
  ...
  try {
    ...
    for (let warm = 0; warm < 3; warm++) getUnifiedThreadInSnapshot(store.db, eligible, { messageId: SELECTED_OLDER_THAN_500 })
    const started = performance.now()
    const detail = getUnifiedThreadInSnapshot(store.db, eligible, { messageId: SELECTED_OLDER_THAN_500 })
    const elapsedMs = Number((performance.now() - started).toFixed(2))
    const plan = explainThreadDetailQueryPlans(store.db, SELECTED_OLDER_THAN_500, eligible)
    ...
    const selected = detail.messages.find((message) => message.selected)
    if (!selected || selected.messageId !== SELECTED_OLDER_THAN_500 || !detail.selectedOutsideRecentWindow || !detail.historyTruncated) {
      throw new Error('thread detail did not prove selected older than the 500-candidate window')
    }
    if (Buffer.byteLength(selected.bodyText, 'utf8') > 64 * 1024 || !selected.bodyTruncated) {
      throw new Error('selected body did not retain its bounded 64KiB reserve')
    }
    if (detail.messages.reduce((sum, message) => sum + Buffer.byteLength(message.bodyText, 'utf8'), 0) > 160 * 1024 ||
        detail.messages.reduce((sum, message) => sum + message.recipients.length, 0) > 64 ||
        detail.messages.reduce((sum, message) => sum + message.attachments.length, 0) > 64) {
      throw new Error('thread detail exceeded global budgets')
    }
    const result = {
      elapsedMs,
